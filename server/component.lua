...
CreateThread(function()
	...
	plsr.Callbacks:RegisterServerCallback("Evidence:Fetch", function(source, data, cb)
		cb(EVIDENCE_CACHE)
	end)
...
RegisterNetEvent("Evidence:Server:PickupEvidence", function(evidenceId)
	...
	if char and plsr.Jobs.Permissions:HasJob(_src, "police") then
		for k, v in ipairs(EVIDENCE_CACHE) do
			if v.id == evidenceId then
				if v.type == "paint_fragment" then
					plsr.Inventory:AddItem(char:GetData("SID"), "evidence-paint", 1, {
						EvidenceType = v.type,
						EvidenceId = v.id,
				...
				elseif v.type == "projectile" then
					plsr.Inventory:AddItem(char:GetData("SID"), "evidence-projectile", 1, {
						EvidenceType = v.type,
						EvidenceId = v.id,
						EvidenceCoords = { x = v.coords.x, y = v.coords.y, z = v.coords.z },
				...
				elseif v.type == "casing" then
					plsr.Inventory:AddItem(char:GetData("SID"), "evidence-casing", 1, {
						EvidenceType = v.type,
						EvidenceId = v.id,
						EvidenceCoords = { x = v.coords.x, y = v.coords.y, z = v.coords.z },
						EvidenceWeapon = v.data and v.data.weapon,
